...
def calculate_green_score(row):
    ...
    material_keywords = {
        'bamboo': 15, 'bagasse': 15, 'plant-based': 15, 'recycled': 10,
        'plastic': -10, 'non-recyclable': -15,
        'hemp': 12, 'cork': 12, 'pinatex': 12, 'apple leather': 12,
        'organic cotton': 12, 'upcycled': 12, 'merino wool': 12, 'vegetable-tanned': 12
    }
    description = row['description'].lower()
    for material, points in material_keywords.items():
        if material in description:
    ...
    label_points = {
        'biodegradable': 10, 'compostable': 10, 'organic': 10,
        'plastic-free': 8, 'vegan': 8, 'ethical': 8, 'recyclable': 7,
        'eco-friendly': 7, 'sustainable': 7, 'upcycled': 7
    }
    labels = row['sustainability_labels'].lower().split(',')
    for label in labels:
        score += label_points.get(label.strip(), 0)

    eco_brands = ['EcoBrush', 'GreenTable', 'NaturePure', 'GreenLeaf', 'EcoWear', 'SteelSip', 'EcoBrew', 'GreenFork',
                  'PureBloom', 'ClearCycle', 'ZenMat', 'CleanWave', 'EcoRest', 'GreenCarry', 'EcoBin', 'TinyGreen',
                  'EcoTech', 'SeaShade', 'PureLeaf', 'EcoSip', 'GreenComb', 'EcoFeet', 'CleanGreen', 'EcoCard',
                  'FreshEco', 'GreenDine', 'EcoDesk', 'GreenPack', 'PureBlend', 'EcoStore', 'SmileGreen', 'CycleWorks',
                  'GlowGreen', 'EcoShade', 'GreenChef', 'EcoRun', 'GreenStep', 'EcoTread']
    if row['brand'] in eco_brands:
        score += 20
    ...
    if row['category'].lower() == 'footwear':
        score += 5
        if any(material in description for material in ['hemp', 'pinatex', 'recycled', 'organic', 'cork', 'rubber', 'wool', 'apple leather']):
            score += 5
...
#print(f"✅ Trained ML Model | Test MSE: {mse:.2f}")
...
def generate_diy_ideas(dimensions):
    ...
    if shape == 'linear' and size == 'small':
        product_reuse.append("Use as plant marker.")
        packaging_reuse.append("Use for pen holder.")
    elif shape == 'volumetric':
        product_reuse.append("Repurpose as small planter.")
        packaging_reuse.append("Use box for storage.")
    elif shape == 'flat' and size == 'small':
        product_reuse.append("Use as coaster.")
        packaging_reuse.append("Use as gift wrap.")

    return product_reuse, packaging_reuse

# --- Predict for New Product (Using ML) ---
def predict_and_generate_ideas(product_data):
    # Vectorize text
    text = product_data['description'] + ' ' + product_data['sustainability_labels']
    text_features = tfidf.transform([text]).toarray()

    # Dimension & numeric features
    shape, size, length, width, height = parse_dimensions(product_data['dimensions'])
    num_features = np.array([[product_data['price'], length, width, height]])
    X_new = np.hstack((text_features, num_features))
...
product_data = {
    ...
    'sustainability_labels': input("Sustainability Labels (comma separated): "),
    'price': float(input("Price: ")),
    'dimensions': input("Dimensions (e.g., 27 x 11 x 7 cm): ")
}

result = predict_and_generate_ideas(product_data)

print(f"\n✅ Product: {result['product_name']}")
print(f"Dimensions: {result['dimensions']}")
print(f"Green Score: {result['green_score']+50} ")
print("♻️ Product Reuse Ideas:")
for idea in result['product_reuse_ideas']:
    print(f"- {idea}")
print("📦 Packaging Reuse Ideas:")
for idea in result['packaging_reuse_ideas']:
